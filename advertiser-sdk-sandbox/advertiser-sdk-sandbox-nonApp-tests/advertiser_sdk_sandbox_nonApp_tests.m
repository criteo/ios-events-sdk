//
//  advertiser_sdk_sandbox_nonApp_tests.m
//  advertiser-sdk-sandbox-nonApp-tests
//
...
@interface advertiser_sdk_sandbox_nonApp_tests : XCTestCase

@end

@implementation advertiser_sdk_sandbox_nonApp_tests
          ...
          testResultCallback:(TestResultsCallback)testResultCallback
{
    ...
    [sender sendEvent:event
     ...
     {
         //NSData* requestBodyData = [item performSelector:@selector(requestBody)];
         //NSData* responseBodyData = [item performSelector:@selector(responseData)];
         //NSLog(@"item sent (s=%d): %@\nResponse: %@",
         //      success,
         //      [[NSString alloc] initWithBytes:requestBodyData.bytes length:requestBodyData.length encoding:NSUTF8StringEncoding],
         //      [[NSString alloc] initWithBytes:responseBodyData.bytes length:responseBodyData.length encoding:NSUTF8StringEncoding]);
